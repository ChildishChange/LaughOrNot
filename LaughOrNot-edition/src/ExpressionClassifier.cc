...
void ExpressionClassifier::load() {
	//string filenames[] = {"neutral.yml","neutral.yml"/*, "smile.yml", "surprise.yml"*/};
	vector<string> filenames;
	filenames.push_back("neutral.yml");
	filenames.push_back("smile.yml");
	filenames.push_back("surprise.yml");
	unsigned int n = filenames.size();
	expressions.resize(n);
	for(unsigned int i = 0; i < n; i++) {
		expressions[i].load(filenames[i].c_str());
	}
}
